...
backend/apps/documents/services/placeholders/litigation/execution_request_interest.py

适配点（去掉 Django/finance 依赖）：
  - `from apps.cases.models import Case`                            → 删除
  - `from apps.finance...InterestCalculator`                        → 本包 .interest_calculator
  - resolve_interest_base(case=...)                                 → resolve_interest_base(target_amount=...)
  - calculate_interest_with_segments 内 PrincipalPeriod 来源         → 本包 .interest_calculator
其余解析逻辑（LPR 倍数/固定年利率/日利率/分段/抵扣顺序）原样保留。
"""
...
def resolve_interest_base(
    *,
    ...
    """等价法穿 resolve_interest_base，把 case.target_amount 替换为显式传入的 target_amount。"""
...
def calculate_interest_with_segments(
    *,
    ...
    from .interest_calculator import PrincipalPeriod
