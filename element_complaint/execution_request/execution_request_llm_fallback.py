...
backend/apps/documents/services/placeholders/litigation/execution_request_llm_fallback.py

适配点：原版通过 Django 的 get_llm_service 调用 Ollama；本独立工具改用 urllib
直连本地 Ollama HTTP API（http://localhost:11434）。未运行 Ollama 时优雅返回 None，
不影响纯规则流程。merge 逻辑（只填空、不覆盖）原样保留。
"""
...
def _call_ollama(prompt: str) -> str:
    """直连本地 Ollama HTTP API，返回模型文本输出。失败返回空串（优雅降级）。"""
    try:
        body = json.dumps({
        ...
        logger.debug("Ollama 不可用，跳过兜底: %s", exc)
        return ""
